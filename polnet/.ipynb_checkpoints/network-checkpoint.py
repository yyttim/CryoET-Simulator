...
class Network(ABC):
    ...
    def perturb_vertices(self, poly_data, scale=0.1):
        ...
        for i in range(num_points):
            ...
            perturbed_point = p + np.random.normal(scale=scale, size=3)
            new_points.InsertNextPoint(perturbed_point)
                    ...
                    if hold_polymer.get_total_len() + new_len < max_length:
                        if single_mode:
                            if hold_polymer.get_num_mmers() == 0:
                                hold_polymer.add_monomer(monomer_data[0], monomer_data[1], monomer_data[2],
                                                         monomer_data[3])
                                self.add_monomer_to_voi(hold_polymer.get_monomer(-1), self._Network__svol)
...
class NetOrgan(Network):
    ...
    def build_network(self, method_type=0, disturb=False):
        """
        Add polymers following SAWLC model until an occupancy limit is passed
        :return:
        """
