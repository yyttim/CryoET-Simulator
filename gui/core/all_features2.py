...
from polnet.affine import get_center_of_polydata
...
def all_features2(NTOMOS, VOI_SHAPE, OUT_DIR, VOI_OFFS, VOI_VSIZE, MMER_TRIES, PMER_TRIES,
                  ...
                  PROTEINS_LIST_DENSITY_LOOP, PROTEINS_LIST_POS_LOOP, PROTEINS_LIST_SINGLE_MODE,
                  PROTEINS_LIST_DISTRIBUTED, ORGANELLE_LIST_DISTRIBUTED,
    ...
    for tomod_id in range(NTOMOS):
        ...
        ORGANELLE_VOI_VSIZE = 1
        organelle_p0 = None
        for p_id, p_file in enumerate(ORGANELLE_LIST):
            print('\tORGANELLE_LIST FILE:', p_file)
            ...
            print(">>>>>>>>>>center", center, model_surf.GetBounds())
            # Monomer centering
            model_surf = pp.poly_translate(model_surf, -center)
            # if p_id == 0:
            #     model_surf = pp.poly_rotate_wxyz(model_surf, 90.0, 0, 0, 1)
            ...
            # ct = get_center_of_polydata(model_surf)
            ...
            p0 = net_sawlc.build_network(2, ORGANELLE_LIST_DISTRIBUTED)
            if p_id == 0:
                organelle_p0 = p0
            ...
            print(f">>>>Organelle insert [{p_file}], cnt={net_sawlc.get_num_mmers()}")
        ...
        for p_id, p_file in enumerate(PROTEINS_LIST):
            ...
            net_sawlc.build_network(PROTEINS_LIST_METHOD[p_id], PROTEINS_LIST_DENSITY_LOOP[p_id], PROTEINS_LIST_POS_LOOP[p_id], PROTEINS_LIST_SINGLE_MODE[p_id], PROTEINS_LIST_DISTRIBUTED[p_id], organelle_p0)
            ...
            protein_cnt = net_sawlc.get_num_mmers()
            print(f">>>>Protein insert [{p_file}], cnt={protein_cnt}")
            if protein_cnt == 0:
                continue
